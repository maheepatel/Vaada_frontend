...
export function PromiseSubmission() {
  ...
  const continueManually = () => {
    if (!session) return toast.error("Please log in before recording a promise.");
    if (!sourceUrl && !file) return toast.error("Add the original source link or upload the original document.");
    setDraft({ ...blank, sourceUrl });
    setStep("review");
    toast.success("Fill in every field from the source yourself.");
  };
  ...
  return <section className="submit-shell"><div className="submit-intro"><p className="eyebrow">RECORD A PROMISE</p><h1>Share the source.<br /><span>Review the draft.</span></h1><p>Start with the original government letter, public announcement, news report or social post. AI prepares an editable draft; a human reviewer decides whether it enters the register.</p><ol><li className={step === "source" ? "active" : ""}>01 · Promise source</li><li className={step === "review" ? "active" : ""}>02 · Check AI draft</li><li className={step === "done" ? "active" : ""}>03 · Private receipt</li></ol></div>
    <div className="intake-panel">{step === "source" && <form onSubmit={extract}><div className="form-heading"><span>STEP 01 · REQUIRED EVIDENCE</span><h2>What proves the promise was made?</h2><p>Add at least one original source: a public link or an uploaded image/PDF. Pasted text is optional context and cannot replace the source.</p></div><label>Public source link<input type="text" value={sourceUrl} onChange={(event) => setSourceUrl(normalizeUrl(event.target.value))} placeholder="Government website, news report or public social post" /></label><div className="source-divider"><span>OR</span></div><EvidenceUpload file={file} onChange={setFile} kind="promise" /><label>Exact source text (optional)<textarea rows={7} value={rawText} onChange={(event) => setRawText(event.target.value)} placeholder="Paste exact wording to help extraction. Do not summarize if the original is available." /></label><div className="ai-boundary"><strong>AI DRAFTS</strong><span>Promise wording, dates, location, deadline and responsible office.</span><strong>HUMANS DECIDE</strong><span>AI never publishes a record, verifies truth or changes public progress.</span></div><div className="form-actions"><button type="button" className="button button-ghost" disabled={busy || (!sourceUrl && !file)} onClick={continueManually}>Skip AI, enter details myself →</button><button className="button button-primary" disabled={busy || (!sourceUrl && !file)}>{busy ? "Reading source…" : "Create editable AI draft →"}</button></div></form>}
