  ...
  return <main className="site-shell">
    ...
    <section className="hero" id="top">
      ...
      <motion.div className="hero-copy" initial={reduceMotion ? false : "hidden"} animate="visible" variants={{ hidden: {}, visible: { transition: { staggerChildren: .12 } } }}>
        <motion.div className="hero-meta" variants={{ hidden: { opacity: 0, y: 16 }, visible: { opacity: 1, y: 0, transition: { duration: .6 } } }}><p className="eyebrow">INDIA&apos;S PUBLIC PROMISE REGISTER</p><span>LIVE RECORD · {String(commitments.length).padStart(3,"0")}</span></motion.div>
        ...
        <motion.div className="hero-bottom" variants={{ hidden: { opacity: 0, y: 26 }, visible: { opacity: 1, y: 0, transition: { duration: .7 } } }}><p>Vaada keeps the public receipt: who promised, what changed, what evidence exists, and how much time remains.</p><div className="hero-actions"><a className="button button-primary" href="#register">Explore promises <span>↓</span></a><a className="button button-ghost" href={mobileAppUrl}>Launch mobile app <span>↗</span></a><Link className="button button-ghost hero-record" href="/submit">Put one on record <span>＋</span></Link></div></motion.div>
      </motion.div>
    ...
    <div className="accountability-marquee" aria-label="Vaada principles"><div>{[0,1].map((copy)=><span aria-hidden={copy === 1} key={copy}><b>NO ACCUSATIONS</b><i />HUMAN REVIEWED<i />SOURCE BACKED<i />PUBLIC BY DESIGN<i /></span>)}</div></div>
    ...
    <section className="stats" aria-label="Register statistics"><article><strong>{String(commitments.length).padStart(2,"0")}</strong><span>public records</span></article><article><strong>{String(stateCounts.length).padStart(2,"0")}</strong><span>states covered</span></article><article><strong>{String(evidenceCount).padStart(2,"0")}</strong><span>sources on record</span></article><article><strong>{completed}</strong><span>promises kept</span></article></section>
    ...
    <section className="final-cta"><p className="eyebrow">PUBLIC MEMORY IS PUBLIC POWER</p><h2>See a promise?<br/>Put it on record.</h2><div className="final-actions"><Link className="button button-invert" href="/submit">Record a promise <span>↗</span></Link><Link className="button button-invert" href="/contact">Contact Vaada <span>↗</span></Link></div></section>
    <SiteFooter/>
  </main>;
}
